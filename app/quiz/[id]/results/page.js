...
export default function QuizResultsPage({ params }) {
    ...
    return (
      <div className="min-h-screen bg-[var(--c-canvas)] flex items-center justify-center">
        <div className="text-center">
          <p className="text-white/60 mb-4">Quiz not found</p>
          <Link href="/quiz" className="text-[#0071E3] hover:underline">
            Back to quizzes
          </Link>
        </div>
      </div>
  ...
  const getGrade = () => {
    if (percentage >= 90) return { grade: 'A*', color: 'text-emerald-400', bg: 'bg-emerald-500/15' };
    if (percentage >= 80) return { grade: 'A', color: 'text-emerald-400', bg: 'bg-emerald-500/15' };
    if (percentage >= 70) return { grade: 'B', color: 'text-[#0071E3]', bg: 'bg-[#0071E3]/12' };
    if (percentage >= 60) return { grade: 'C', color: 'text-[#0071E3]', bg: 'bg-[#0071E3]/12' };
    if (percentage >= 50) return { grade: 'D', color: 'text-orange-400', bg: 'bg-orange-500/15' };
    return { grade: 'E', color: 'text-red-400', bg: 'bg-red-500/15' };
  };
  ...
  const getLevelBar = (score, level) => {
    const colors = {
      easy: { bg: 'bg-emerald-500', light: 'bg-white/8' },
      medium: { bg: 'bg-[#0071E3]', light: 'bg-[#0071E3]/12' },
      hard: { bg: 'bg-red-500', light: 'bg-white/8' }
    };
    ...
    return (
      <div className="flex items-center gap-3">
        <span className="text-sm font-medium text-white/60 w-16 capitalize">{level}</span>
        ...
        <span className="text-sm font-bold text-white/70 w-16 text-right">{displayEarned}/{displayTotal}</span>
      </div>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-[var(--c-canvas)]">
      {/* Header */}
      <header className="bg-[var(--c-card)] border-b border-[var(--c-border)]">
        <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
          <Link href="/quiz" className="p-2 -ml-2 hover:bg-white/5 rounded-lg transition-colors">
            <svg className="w-5 h-5 text-white/60" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
            </svg>
          </Link>
          <h1 className="font-semibold text-white">Quiz Results</h1>
          <div className="w-9" />
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
        {/* Score Card */}
        <div className="bg-[var(--c-card)] rounded-2xl border border-[var(--c-border)] p-8 text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-white mb-1">{marksEarned} / {quizTotalMarks} marks</h2>
          <p className="text-lg text-white/50 mb-4">{percentage}%</p>
          <p className="text-white/60">{getMessage()}</p>
        </div>

        {/* Topic Info */}
        <div className="bg-[var(--c-card)] rounded-xl border border-[var(--c-border)] p-4 mb-6">
          <div className="flex items-center justify-between">
            <div>
              <h3 className="font-semibold text-white">{quiz.topicName}</h3>
              <p className="text-sm text-white/50">{quiz.subject}</p>
            </div>
            {quiz.completedAt && (
        ...
        {/* Level Breakdown */}
        <div className="bg-[var(--c-card)] rounded-xl border border-[var(--c-border)] p-6 mb-6">
        ...
        {(quiz.easyScore < 4 || quiz.mediumScore < 3 || quiz.hardScore < 2) && (
          <div className="bg-[#0071E3]/10 border border-[#0071E3]/20 rounded-xl p-4 mb-6">
            <div className="flex items-start gap-3">
              <svg className="w-5 h-5 text-[#0071E3] mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
              </svg>
              <div>
                <h4 className="font-semibold text-[#0071E3] mb-1">Areas to Review</h4>
        ...
        {flaggedQuestions.length > 0 && (
          <div className="bg-[var(--c-card)] rounded-xl border border-[var(--c-border)] p-6 mb-6">
            <div className="flex items-center gap-2 mb-4">
              <svg className="w-5 h-5 text-[#0071E3]" fill="currentColor" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3 3v1.5M3 21v-6m0 0l2.77-.693a9 9 0 016.208.682l.108.054a9 9 0 006.086.71l3.114-.732a48.524 48.524 0 01-.005-10.499l-3.11.732a9 9 0 01-6.085-.711l-.108-.054a9 9 0 00-6.208-.682L3 4.5M3 15V4.5" />
              </svg>
              <h3 className="font-semibold text-white">Flagged for Review</h3>
              <span className="ml-auto text-sm text-white/50">{flaggedQuestions.length} question{flaggedQuestions.length !== 1 ? 's' : ''}</span>
            </div>
            <div className="space-y-2 mb-4">
              {flaggedQuestions.map((q, idx) => (
                <div key={idx} className="flex items-start gap-3 p-3 bg-[var(--bg-surface)] rounded-lg">
                  <span className={`text-xs font-medium px-2 py-0.5 rounded capitalize flex-shrink-0 ${
                    q.level === 'easy' ? 'bg-emerald-500/12 text-emerald-400' :
                    q.level === 'medium' ? 'bg-[#0071E3]/12 text-[#0071E3]' :
                    'bg-red-500/12 text-red-400'
                  }`}>
                    {q.level}
                  </span>
                  <p className="text-sm text-white/70 flex-1"><MathText>{q.questionText}</MathText></p>
                </div>
              ))}
            </div>
            <Link
              ...
              className="flex items-center justify-center gap-2 w-full py-3 bg-[#0071E3] hover:bg-[#0071E3] text-white font-medium rounded-xl transition-colors"
            >
        ...
        <div className="space-y-3 mb-8">
          <button
            ...
            className="flex items-center justify-center gap-2 w-full py-3 bg-white/6 hover:bg-white/8 text-white font-medium rounded-xl transition-colors border border-[var(--c-border)]"
          >
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
          ...
          <Link
            ...
            className="flex items-center justify-center gap-2 w-full py-3 bg-[#0071E3] hover:bg-[#0058B3] text-white font-medium rounded-xl transition-colors"
          >
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M8.625 12a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H8.25m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0H12m4.125 0a.375.375 0 11-.75 0 .375.375 0 01.75 0zm0 0h-.375M21 12c0 4.556-4.03 8.25-9 8.25a9.764 9.764 0 01-2.555-.337A5.972 5.972 0 015.41 20.97a5.969 5.969 0 01-.474-.065 4.48 4.48 0 00.978-2.025c.09-.457-.133-.901-.467-1.226C3.93 16.178 3 14.189 3 12c0-4.556 4.03-8.25 9-8.25s9 3.694 9 8.25z" />
            </svg>
          ...
          <Link
            href="/quiz"
            className="flex items-center justify-center gap-2 w-full py-3 bg-[var(--c-card)] border border-[var(--c-border)] hover:bg-[var(--bg-surface)] text-white/70 font-medium rounded-xl transition-colors"
          >
            Back to Quizzes
          </Link>
        </div>
        ...
        {showReview && (
          <div className="bg-[var(--c-card)] rounded-xl border border-[var(--c-border)] overflow-hidden">
            <div className="p-4 border-b border-[var(--c-border)] bg-[var(--bg-surface)]">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 text-sm font-medium rounded-lg transition ${
                      reviewFilter === filter.id
                        ...
                        : 'bg-[var(--c-card)] border border-[var(--c-border)] text-white/60 hover:border-[var(--c-border-strong)]'
                    }`}
                  >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1">
                          <span className={`text-xs font-medium px-2 py-0.5 rounded capitalize ${
                            question.level === 'easy' ? 'bg-emerald-500/12 text-emerald-400' :
                            question.level === 'medium' ? 'bg-[#0071E3]/12 text-[#0071E3]' :
                            'bg-red-500/12 text-red-400'
                          }`}>
                            {question.level}
                          </span>
                                  ...
                                  className={`px-3 py-2 rounded-lg text-sm ${
                                    isCorrectOption
                                      ...
                                      : isStudentAnswer && !question.isCorrect
                                        ...
                                        : 'bg-[var(--bg-surface)] text-white/60'
                                  }`}
                                >
                                  <MathText>{option}</MathText>
                                  {isCorrectOption && <span className="ml-2 text-emerald-400 font-medium">✓ Correct</span>}
                                  {isStudentAnswer && !isCorrectOption && <span className="ml-2 text-red-400 font-medium">Your answer</span>}
                                </div>
                              );
                            })}
                          </div>
                        ...
                        {question.questionType !== 'multiple_choice' && (
                          <div className="space-y-2 mb-3">
                            ...
                            {question.skipped && (
                              <div className="px-3 py-2 rounded-lg text-sm bg-white/5 text-white/60">
                                <span className="font-medium">Skipped</span>
                              </div>
                            )}
                            <div className="px-3 py-2 rounded-lg text-sm bg-emerald-500/8 border border-emerald-500/20 text-emerald-300">
                        ...
                        {question.explanation && (
                          <div className="px-3 py-2 rounded-lg text-sm bg-[#0071E3]/8 border border-[#0071E3]/15 text-white/60">
