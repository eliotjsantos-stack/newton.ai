...
function ForensicPanel({ student, onClose }) {
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 bottom-0 w-96 z-50 border-l border-white/8 overflow-y-auto bg-[var(--c-card)] animate-slide-in-right">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/8">
          <div>
            <h2 className="text-base font-bold text-white">{student.name}</h2>
            <p className="text-xs text-white/50 mt-0.5">Student Forensic Profile</p>
          </div>
          <button onClick={onClose} className="text-white/40 hover:text-white/60 p-1 transition-colors">
        ...
        <div className="p-6 space-y-6">
          {/* Knowledge Half-Life */}
          <div className="rounded-3xl border border-white/8 bg-[var(--bg-surface)] p-4">
            ...
            <p className="text-xs text-white/50 mt-2">Estimated retention drops below 50% in <span className="text-[#0071E3] font-semibold">{halfLife}</span> days</p>
          </div>

          {/* Syllabus Velocity */}
          <div className="rounded-3xl border border-white/8 bg-[var(--bg-surface)] p-4">
            <h3 className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-2">Syllabus Velocity</h3>
            <p className="text-2xl text-white font-semibold">{syllabusVelocity} <span className="text-sm text-white/50">min/node</span></p>
            <p className="text-xs text-white/50 mt-1">Average time per syllabus node</p>
          </div>

          {/* Refusal Count */}
          <div className="rounded-3xl border border-white/8 bg-[var(--bg-surface)] p-4">
            <h3 className="text-xs font-semibold text-white/40 uppercase tracking-wider mb-2">Refusal Count</h3>
            <p className="text-2xl text-[#0071E3] font-semibold">{refusalCount}</p>
            <p className="text-xs text-white/50 mt-1">Direct answer requests blocked</p>
          </div>
        </div>
      </div>
    </>
...
function QuizDetailPanel({ studentId, studentName, chapter, classId, onClose }) {
  ...
  const statusDot = mastery?.status === 'green' ? 'bg-emerald-400' : mastery?.status === 'amber' ? 'bg-[#0071E3]' : 'bg-red-400';

  return (
    <>
      <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-40" onClick={onClose} />
      <div className="fixed right-0 top-0 bottom-0 w-[28rem] z-50 border-l border-white/8 overflow-y-auto bg-[var(--c-card)] animate-slide-in-right">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-white/8">
          <div className="min-w-0">
            <h2 className="text-base font-bold text-white truncate">{studentName}</h2>
            <p className="text-xs text-white/50 mt-0.5 truncate">{chapter}</p>
          </div>
          <button onClick={onClose} className="text-white/40 hover:text-white/60 p-1 transition-colors shrink-0">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
            </svg>
          </button>
          ...
          {loading ? (
            ...
            <>
              ...
              {(summary || summaryLoading) && (
                <div className="rounded-2xl border border-[#0071E3]/20 bg-[#0071E3]/10 p-4">
              ...
              {/* Mastery Status */}
              {mastery && (
                <div className="rounded-2xl border border-white/8 bg-[var(--bg-surface)] p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-white/50">Last quiz: {formatDate(mastery.last_quiz_at)}</p>
                  </div>
                </div>
              )}
              ...
              {/* Quiz List */}
              {quizzes.length === 0 ? (
                <div className="rounded-2xl border border-white/8 bg-[var(--bg-surface)] p-6 text-center">
                  <p className="text-sm text-white/50">No quizzes found for this topic.</p>
                </div>
              ) : (
                <div className="space-y-3">
                  ...
                  {quizzes.map((quiz) => {
                    ...
                    return (
                      <div key={quiz.id} className="rounded-2xl border border-white/8 bg-[var(--c-card)] overflow-hidden">
                        <button
                          onClick={() => setExpandedQuiz(isExpanded ? null : quiz.id)}
                          className="w-full px-4 py-3 flex items-center justify-between hover:bg-[var(--bg-surface)] transition-colors text-left"
                        >
                          <div>
                          ...
                          <div className="flex items-center gap-3">
                            <div className="text-right">
                              <p className={`text-sm font-semibold ${pct >= 70 ? 'text-emerald-500' : pct >= 40 ? 'text-[#0071E3]' : 'text-red-500'}`}>
                                {earned}/{total}
                              </p>
                        ...
                        {isExpanded && quiz.questions && (
                          <div className="border-t border-white/8 divide-y divide-gray-100">
                                ...
                                <div key={qi} className="px-4 py-3">
                                  <div className="flex items-start gap-2">
                                    <span className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold shrink-0 ${
                                      isCorrect ? 'bg-emerald-100 text-emerald-600' : isPartial ? 'bg-[#0071E3]/12 text-[#0071E3]' : 'bg-red-100 text-red-600'
                                    }`}>
                                      {marksAwarded}
                                    </span>
                                    <div className="min-w-0 flex-1">
                                      <p className="text-xs text-white/60 leading-relaxed">{q.question || q.text || `Question ${qi + 1}`}</p>
                                      {answer?.student_answer && (
                                        <p className="text-xs text-white/50 mt-1">
                                          <span className="text-white/30">Student:</span> {answer.student_answer}
                                        </p>
                                      )}
                                      {(q.correct_answer || q.answer) && (
                                        <p className="text-xs text-emerald-600/70 mt-0.5">
                                          <span className="text-white/30">Correct:</span> {q.correct_answer || q.answer}
                                        </p>
                                      )}
                                      <p className="text-[10px] text-white/40 mt-1">{marksAwarded}/{marksAvailable} marks</p>
                                    </div>
                                  </div>
                                </div>
                              );
...
function AssignQuizModal({ classId, className, subject, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="rounded-3xl shadow-2xl max-w-md w-full max-h-[80vh] overflow-y-auto bg-[var(--c-card)] border border-white/8 shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b border-white/8 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-bold text-white">Assign Quiz</h3>
            <p className="text-sm text-white/50 mt-0.5">{className} &middot; {subject}</p>
          </div>
          <button onClick={onClose} className="text-white/40 hover:text-white/60 p-1 transition-colors">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
          ...
          {result ? (
            result.success ? (
              <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-6 text-center">
                ...
                <p className="text-xs text-white/50 mt-1">Students will see it in their quiz hub.</p>
                <button onClick={onClose} className="mt-4 px-6 py-2 text-xs font-semibold bg-white/5 hover:bg-white/8 text-white/70 rounded-full transition-colors border border-white/8">
                  Done
                </button>
              </div>
            ) : (
              <div className="bg-red-50 border border-red-200 rounded-2xl p-6 text-center">
                ...
                <p className="text-xs text-white/50 mt-1">Please try again.</p>
                <button onClick={() => setResult(null)} className="mt-4 px-6 py-2 text-xs font-semibold bg-white/5 hover:bg-white/8 text-white/70 rounded-full transition-colors border border-white/8">
                  Try Again
                </button>
              </div>
            )
            ...
            <>
              {/* Topic */}
              <div>
                <label className="text-sm font-medium text-white/50 block mb-1.5">Topic</label>
                <input
                  type="text"
                  value={topic}
                  onChange={e => setTopic(e.target.value)}
                  placeholder="e.g. Quadratic equations"
                  className="w-full px-4 py-2.5 bg-[var(--bg-surface)] border border-gray-300 rounded-xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-[#0071E3]/30 placeholder:text-white/40"
                />
              </div>

              {/* Mode */}
              <div>
                <label className="text-sm font-medium text-white/50 block mb-1.5">Mode</label>
                  ...
                  {modes.map(m => (
                    <button
                      ...
                      className={`px-3 py-1.5 text-xs font-semibold rounded-full transition-colors border ${
                        mode === m.value
                          ? 'bg-[#0071E3] text-whiteorder-amber-200'
              ...
              {/* Total Marks */}
              <div>
                <label className="text-sm font-medium text-white/50 block mb-1.5">Total Marks: {totalMarks}</label>
                <div className="flex items-center gap-3">
                  <button
                    onClick={() => setTotalMarks(Math.max(5, totalMarks - 5))}
                    className="w-8 h-8 rounded-full bg-white/5 border border-white/8 text-white/50 hover:text-white/70 flex items-center justify-center transition-colors"
                  >
                    -
                  </button>
              ...
              {/* Due Date */}
              <div>
                <label className="text-sm font-medium text-white/50 block mb-1.5">Due Date (optional)</label>
                <input
                  type="date"
                  value={dueDate}
                  onChange={e => setDueDate(e.target.value)}
                  className="w-full px-4 py-2.5 bg-[var(--bg-surface)] border border-gray-300 rounded-xl text-sm text-white focus:outline-none focus:ring-2 focus:ring-[#0071E3]/30"
                />
              </div>
              ...
              <button
                ...
                className="w-full py-2.5 bg-[#0071E3] text-whitetext-sm font-semibold rounded-full hover:bg-[#0058B3] disabled:opacity-50 transition"
              >
...
function InterventionModal({ student, className, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm flex items-center justify-center p-4" onClick={onClose}>
      <div className="rounded-3xl shadow-2xl max-w-lg w-full max-h-[80vh] overflow-y-auto bg-[var(--c-card)] border border-white/8 shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="p-6 border-b border-white/8 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-bold text-white">Intervene — {student.name}</h3>
            <p className="text-sm text-white/50 mt-0.5">{className}</p>
          </div>
          <button onClick={onClose} className="text-white/40 hover:text-white/60 p-1 transition-colors">
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" />
        ...
        <div className="p-6">
          <div className="bg-[#0071E3]/100/10 border border-amber-500/20 rounded-2xl p-4 mb-6">
          ...
          {!mode && (
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className="flex flex-col items-center gap-2 p-5 rounded-2xl border border-white/8 bg-[var(--bg-surface)] hover:border-[#0071E3]/20 hover:bg-[#0071E3]/10 transition text-center"
              >
                ...
                <span className="text-xs text-white/40">AI-generated revision sheet</span>
              </button>
              <button
                onClick={() => setMode('notify')}
                className="flex flex-col items-center gap-2 p-5 rounded-2xl border border-white/8 bg-[var(--bg-surface)] hover:border-[#0071E3]/25 hover:bg-[#0071E3]/10 transition text-center"
              >
                <svg className="w-8 h-8 text-[#0071E3]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                ...
                <span className="text-xs text-white/40">Send progress alert email</span>
              </button>
            </div>
          )}
          ...
          {mode === 'worksheet' && !worksheet && (
            <div className="space-y-4">
              ...
              <div>
                <label className="text-sm font-medium text-white/50 block mb-1">Topic</label>
                <input
                  type="text"
              ...
              <div>
                <label className="text-sm font-medium text-white/50 block mb-1">Difficulty</label>
                <select
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-3 max-h-60 overflow-y-auto">
                {worksheet.questions?.map((q, i) => (
                  <div key={i} className="p-3 rounded-2xl border border-white/8 bg-[var(--bg-surface)]">
                    <div className="flex justify-between items-start gap-2">
                      <p className="text-sm text-white/70"><strong>Q{q.number}.</strong> {q.question}</p>
              ...
              <button
                ...
                className="w-full py-2.5 bg-white/5 text-white/70 text-sm font-semibold rounded-full hover:bg-white/8 transition border border-white/8"
              >
                Copy to Clipboard
              </button>
            </div>
              ...
              {notifyResult === 'sent' ? (
                ...
                <>
                  ...
                  <button
                    ...
                    disabled={notifying}
                    className="w-full py-2.5 bg-[#0071E3] text-whitetext-sm font-semibold rounded-full hover:bg-[#0071E3] disabled:opacity-50 transition"
                  >
...
function GlassTooltip({ active, payload, label }) {
  ...
  return (
    <div className="rounded-xl bg-[var(--c-card)] border border-white/8 px-3 py-2 shadow-lg">
...
function KpiCard({ label, value, suffix, icon }) {
  return (
    <div className="bg-[var(--c-card)] border border-white/8 rounded-3xl p-6">
  ...
  return (
    <RoleWrapper>
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        {loading ? (
          <div className="space-y-4">
            <div className="h-10 w-48 bg-[var(--c-card)] rounded-xl animate-pulse" />
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {[...Array(3)].map((_, i) => <div key={i} className="h-32 bg-[var(--c-card)] rounded-3xl animate-pulse" />)}
            </div>
          </div>
        ) : classes.length === 0 ? (
          <div className="pt-12">
            <h1 className="text-2xl font-bold text-whitetracking-tight mb-2">Analytics</h1>
            <div className="rounded-3xl border border-white/8 bg-[var(--c-card)] p-12 text-center">
          ...
          /* ── Class Picker ── */
          <div>
            <h1 className="text-2xl font-bold text-whitetracking-tight">Analytics</h1>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {classes.filter(c => !c.archived).map(c => (
                <button
                  ...
                  className="text-left rounded-3xl border border-white/8 bg-[var(--bg-surface)] hover:bg-white/5 p-6 transition-colors group"
                >
                  <div className="flex items-center gap-3 mb-3">
                    <div
                      className="w-10 h-10 rounded-xl flex items-center justify-center text-whitetext-lg"
                      style={{ backgroundColor: c.color || '#3B82F6' }}
                    >
                      {c.subject?.[0]?.toUpperCase() || '?'}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold text-whitetruncate">{c.name}</p>
                      <p className="text-xs text-white/40 truncate">{c.subject} &middot; {c.year_group}</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-white/40">{c.student_count || 0} students</span>
          ...
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              <button onClick={() => { setSelectedClassId(null); setAnalytics(null); }} className="p-1.5 rounded-lg hover:bg-white/5 transition-colors">
              ...
              <div className="h-8 w-48 bg-[var(--c-card)] rounded-xl animate-pulse" />
            </div>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {[...Array(4)].map((_, i) => <div key={i} className="h-28 bg-[var(--c-card)] rounded-3xl animate-pulse" />)}
            </div>
            <div className="h-64 bg-[var(--c-card)] rounded-3xl animate-pulse" />
          </div>
        ) : analytics ? (
          <>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-2xl font-bold text-whitetracking-tight">{analytics.className || 'Command Center'}</h1>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <button
                  ...
                  className="px-4 py-2 text-xs font-semibold bg-[#0071E3] hover:bg-[#0058B3] text-whiterounded-full transition-colors"
                >
                  Assign Quiz
                </button>
                <button
                  onClick={() => handleExportPDF()}
                  className="px-4 py-2 text-xs font-semibold bg-[var(--c-card)] hover:bg-white/5 text-whiteorder border-white/8 rounded-full transition-colors"
                >
                  Download PDF
                </button>
                <button
                ...
                {classes.length > 1 && (
                  <select
                    ...
                    className="px-4 py-2 bg-[var(--bg-surface)] border border-gray-300 rounded-full text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-[#0071E3]/30"
                  >
                    {classes.map(c => (
            ...
            <div className="flex gap-1 bg-[var(--c-card)] border border-white/8 rounded-full p-1 w-fit">
              {tabs.map(tab => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`px-4 py-2 text-xs font-semibold rounded-full transition-colors capitalize ${
                    activeTab === tab ? 'bg-white/5 text-white' : 'text-white/40 hover:text-white/60'
                  }`}
                >
            ...
            {activeTab === 'heatmap' && (
              <div className="space-y-4">
                ...
                />
                <div className="flex items-center justify-between rounded-3xl border border-white/8 bg-[var(--c-card)] p-4">
                ...
                <div className="grid grid-cols-12 gap-4">
                  {/* Mastery Over Time */}
                  <div className="col-span-12 lg:col-span-7 bg-[var(--c-card)] border border-white/8 rounded-3xl p-6">
                    <h3 className="text-sm font-semibold text-whitemb-4">Mastery Over Time</h3>
                    {masteryOverTimeData.length > 0 ? (
                      ...
                      <div className="h-60 flex items-center justify-center text-white/40 text-sm">No quiz data to chart yet</div>
                    )}
                  </div>

                  {/* Topic Distribution */}
                  <div className="col-span-12 lg:col-span-5 bg-[var(--c-card)] border border-white/8 rounded-3xl p-6">
                    <h3 className="text-sm font-semibold text-whitemb-4">Topic Distribution</h3>
                    {topicDistributionData.length > 0 ? (
                      ...
                      <div className="h-60 flex items-center justify-center text-white/40 text-sm">No topic data yet</div>
                    )}
                  </div>
                </div>
              </div>
            ...
            {activeTab === 'priority' && (
              <div>
                {selectedStudents.size > 0 && (
                  <div className="flex items-center gap-2 p-3 bg-[var(--c-card)] border border-white/8 rounded-3xl mb-4">
                    <span className="text-xs text-white/40">{selectedStudents.size} selected</span>
                    <button onClick={handleNudge} disabled={nudging} className="px-3 py-1.5 text-xs font-semibold bg-[#0071E3] hover:bg-[#0071E3] text-whiterounded-full transition-colors disabled:opacity-50">
                      {nudging ? 'Sending...' : 'Nudge Selected'}
                    </button>
                    <button onClick={() => handleExportPDF()} className="px-3 py-1.5 text-xs font-semibold bg-[var(--c-card)] hover:bg-white/5 text-whiterounded-full transition-colors border border-white/8">
                      Export Selected
                    </button>
                    <button onClick={() => setSelectedStudents(new Set())} className="px-3 py-1.5 text-xs text-white/40 hover:text-white/70 transition-colors">
                      Clear
                    </button>
                  </div>
                )}
                    ...
                    <div className="flex items-center justify-between mb-4">
                      ...
                      <label className="flex items-center gap-2 text-xs text-white/40 cursor-pointer">
                        <input
                          ...
                          className="rounded border-gray-300 bg-[var(--bg-surface)]"
                        />
                        Select All
                      </label>
                    </div>
                    <div className="rounded-3xl border border-white/8 bg-[var(--c-card)] overflow-hidden divide-y divide-gray-100">
                      {analytics.priorityList.map((student) => (
                        <div key={student.id} className="px-5 py-4 flex items-center justify-between hover:bg-[var(--bg-surface)] transition-colors">
                          <div className="flex items-center gap-3">
                            <input
                              type="checkbox"
                              ...
                              className="rounded border-gray-300 bg-[var(--bg-surface)]"
                            />
                            <div className="w-9 h-9 rounded-full bg-[var(--c-card)] border border-white/8 flex items-center justify-center">
                            ...
                            <div>
                              <button
                                ...
                                className="text-sm font-semibold text-whitehover:text-[#0071E3] transition-colors text-left"
                              >
                                {student.name}
                              </button>
                          ...
                          <div className="flex items-center gap-3">
                            {student.decayCount > 0 && (
                              <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-[#0071E3]/100/10 text-[#60a5fa]">
                            ...
                            <button
                              ...
                              className="px-3 py-1 rounded-full text-xs font-semibold bg-[#0071E3]/10 text-[#0071E3] hover:bg-[#0071E3]/12 transition"
                            >
                              Intervene
                            </button>
                          </div>
            ...
            {activeTab === 'evidence' && (
              <div className="rounded-3xl border border-white/8 bg-[var(--c-card)] overflow-hidden">
                <div className="px-5 py-4 border-b border-white/8">
                  <h2 className="text-sm font-semibold text-white/50 uppercase tracking-wider">Evidence of Progress</h2>
                  <p className="text-xs text-white/40 mt-1">Measurable learning gains across the class</p>
                </div>
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {(analytics.students || []).map((s, i) => {
                      ...
                      return (
                        <tr key={s.id || i} className="border-b border-white/5 hover:bg-[var(--bg-surface)]">
                          ...
                          <td className="px-3 py-3 text-xs text-whitefont-semibold">{currentGrade}</td>
                          <td className={`px-3 py-3 text-xs font-semibold ${change > 0 ? 'text-emerald-400' : change < 0 ? 'text-red-400' : 'text-white/40'}`}>
                ...
                {(analytics.students || []).length > 0 && (() => {
                  ...
                  return (
                    <div className="px-5 py-4 border-t border-white/8 flex gap-6">
                      <div>
                        <p className="text-[10px] text-white/40 uppercase tracking-wider">Students Improved</p>
                        <p className="text-lg text-emerald-400 font-semibold">{Math.round((improved / students.length) * 100)}%</p>
                      </div>
                      <div>
                        <p className="text-[10px] text-white/40 uppercase tracking-wider">Avg Grade Shift</p>
                        <p className="text-lg text-whitefont-semibold">+{avgShift}</p>
                      </div>
                    </div>
                  );
                })()}
