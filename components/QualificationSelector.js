    ...
    <div ref={containerRef} className="relative">
      ...
      {selectedQual && !isOpen ? (
        <div
          ...
          className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl cursor-pointer hover:bg-gray-100 transition-colors border border-gray-200"
        >
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-10 h-10 bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl flex items-center justify-center shrink-0">
            ...
            <div className="min-w-0">
              ...
              <div className="flex items-center gap-2 mt-0.5">
                <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md ${BOARD_COLORS[selectedQual.board] || 'bg-gray-100 text-gray-600'}`}>
                  {selectedQual.board}
                </span>
        ...
        <div
          className={`flex items-center gap-2 px-4 py-3 rounded-2xl border transition-all duration-200 ${
            isOpen
              ? 'bg-white border-gray-300 shadow-lg ring-2 ring-gray-900/5'
      ...
      {/* Dropdown */}
      {isOpen && (
        <div className="absolute z-50 mt-2 w-full bg-white rounded-2xl border border-gray-200 shadow-2xl overflow-hidden animate-scaleIn"
          style={{ maxHeight: '320px', boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}
        >
          <div className="overflow-y-auto" style={{ maxHeight: '320px' }}>
            {loading ? (
              <div className="p-6 text-center text-sm text-gray-500">Loading courses...</div>
            ) : totalResults === 0 ? (
              ...
              grouped.map((group) => (
                <div key={group.label}>
                  <div className="px-4 py-2 text-[10px] font-bold uppercase tracking-wider text-gray-400 bg-gray-50 sticky top-0">
                  ...
                  {group.items.map((qual) => (
                    <button
                      ...
                      className={`w-full px-4 py-3 text-left flex items-center gap-3 hover:bg-gray-50 transition-colors ${
                        value === qual.qan_code ? 'bg-gray-100' : ''
                      }`}
                    >
                      ...
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-md shrink-0 ${BOARD_COLORS[qual.board] || 'bg-gray-100 text-gray-600'}`}>
                        {qual.board}
                      </span>
                    </button>
                  ))}
